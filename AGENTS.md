...
- `MealSwapModal` is a full-screen overlay at `z-[70]`, which MUST stay above the fixed `BottomNavigation` (`z-[60]`); otherwise the nav overlaps and intercepts taps on the footer "Swap Meal" button, so a swap can't be completed on mobile even though the backend applies it. Keep this ordering if either z-index changes.
