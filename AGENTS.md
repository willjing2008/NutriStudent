...
- `src/app/utils/eventsToClasses.ts` is the pure, unit-tested mapper (occurrence → `ClassEntry`). EventKit returns expanded weekly occurrences for a date range, so **no RRULE parsing** — it just reads each occurrence's local day/time. It drops all-day, blank-title, degenerate, cross-midnight and ≥16-hour events, and scopes by a chosen calendar-id set. Sharp edge: the plugin's iOS bridge serializes the all-day flag as `allDay` while the plugin's TS interface declares `isAllDay` — the mapper must check both keys (checking only `isAllDay` imported all-day events as fake 12am–11:59pm classes, P1, July 2026).
