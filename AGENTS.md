...
- Release build must set `VITE_REVENUECAT_IOS_API_KEY` (prod iOS public key) at build time or `revenuecat.ts` ships the sandbox fallback key; prod Supabase must set `REVENUECAT_SECRET_KEY` or the server paywall fails open. Neither is committed.
